...
const logFile = path.join(logsDir, 'tasks-service.log')
...
const writeLog = async (level, message, meta = {}) => {
  try {
    ...
    const output = `[tasks-service] ${level.toUpperCase()} ${message}`
    if (level === 'error') {
    ...
    console.error('[tasks-service] LOGGING_FAILED', { error: error.message, level, message })
  }
}
