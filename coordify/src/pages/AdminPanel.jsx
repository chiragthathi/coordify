...
export const AdminPanel = () => {
  ...
  const membersAsync = useAsync(teamService.getMembers)
  ...
  const loadMembers = async () => {
    const result = await membersAsync.execute({})
    if (result.success) {
  ...
  const confirmRemoveMember = async () => {
    ...
    const result = await removeMemberAsync.execute(removeCandidate)
    if (result.success) {
  ...
  return (
    <div className="space-y-8">
      ...
      <AiConfirmDialog
        ...
        busy={removeMemberAsync.loading}
      />
      ...
      <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-6 flex items-start gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-yellow-800 dark:text-yellow-300">
            Ensure all user management operations are properly authenticated,
            audited, and comply with your security policies.
          </p>
        </div>
      </div>
    </div>
